...
function setArrays(){
	if($("#stats-chart").length)
	{
		...
		for(loopTime = startTime; loopTime < endTime; loopTime += 86400000)
		{
			...
			window.mingochart[i] = [loopDay,mingo[i]];
			window.skolchart[i] = [loopDay,skol[i]];
			window.mondchart[i]  = [loopDay,mond[i]];
			i++;
		}
	}
	if($("#stats-chart2").length)
	{
		...
		var startTime1 = date3.getTime(), endTime1 = date4.getTime();
		i = 0;
		for(loopTime1 = startTime1; loopTime1 < endTime1; loopTime1 += 86400000)
		{
			var loopDay1=new Date(loopTime1)
    		//use loopDay as you wish
			window.lastmonth[i] = [loopDay1,lastm[i]];
			window.currentmonth[i] = [loopDay1,currentm[i]];
			i++;
		}
	}
	if($("#stackchart").length)
	{
		...
		for(loopTime = startTime; loopTime < endTime; loopTime += 86400000)
		{
			var loopDay=new Date(loopTime)
    		//use loopDay as you wish
			window.t1[i] = [loopDay,trans1[i]];
			window.t2[i] = [loopDay,trans2[i]];
			window.t3[i]  = [loopDay,trans3[i]];
			i++;
		}
	}
...
function template_functions(){
	...
	$('.floatleft').click(function(e){
		...
		if($(this).children().hasClass('fa-icon-sort-up')) $(this).children().removeClass('fa-icon-sort-up').addClass('fa-icon-sort-down');
		else if($(this).children().hasClass('fa-icon-sort-down')) $(this).children().removeClass('fa-icon-sort-down').addClass('fa-icon-sort-up');
		else{
		 $(this).children().removeClass('fa-icon-sort').addClass('fa-icon-sort-up');
		 	if ($(this).siblings().children().hasClass('fa-icon-sort-down'))
		 		$(this).siblings().children().removeClass('fa-icon-sort-down').addClass('fa-icon-sort');
		 	else if ($(this).siblings().children().hasClass('fa-icon-sort-up'))
		 		$(this).siblings().children().removeClass('fa-icon-sort-up').addClass('fa-icon-sort');}
		$target.slideToggle();




		//if($('i',$(this)).hasClass('fa-icon-sort-up')) $('i',$(this)).removeClass('fa-icon-sort-up').addClass('fa-icon-sort-down');
		//else if($('i',$(this)).hasClass('fa-icon-sort-down')) $('i',$(this)).removeClass('fa-icon-sort-down').addClass('fa-icon-sort-up');
		//else{
		// $('i',$(this)).removeClass('fa-icon-sort').addClass('fa-icon-sort-up');
		// 	if ($('i',$(this)).parent().parent().siblings().children().children().hasClass('fa-icon-sort-down'))
		// 		$('i',$(this)).parent().parent().siblings().children().children().removeClass('fa-icon-sort-down').addClass('fa-icon-sort');
		// 	else if ($('i',$(this)).parent().parent().siblings().children().children().hasClass('fa-icon-sort-up'))
		// 		$('i',$(this)).parent().parent().siblings().children().children().removeClass('fa-icon-sort-up').addClass('fa-icon-sort');}
		//$target.slideToggle();
	});
...
function charts() {
	...
	if($("#stats-chart").length)
	{
		var plot = $.plot($("#stats-chart"),
			   [ { data: mingochart, label: lab1}, { data: skolchart, label: lab2 }, { data: mondchart, label: lab3} ], {
				   series: {
					   lines: { show: true,
								lineWidth: 3,
	...
	if($("#stats-chart2").length)
	{
		var plot = $.plot($("#stats-chart2"),
			   [ { data: lastmonth, label: labm}, { data: currentmonth, label: labc }], {
				   series: {
					   lines: { show: true,
								lineWidth: 3,
								fill: true, fillColor: { colors: [ { opacity: 0.08 }, { opacity: 0.01 } ] }
	...
	if($("#stackchart").length)
	{
		...
		function plotWithOptions() {
			$.plot($("#stackchart"), [ { data: t1, label: lab4}, { data: t2, label: lab5 }, { data: t3, label: lab6}], {
				series: {
					stack: stack,
					lines: { show: lines, fill: true, steps: steps },
					bars: { show: bars, barWidth: 60*60*1000*10},
		...
		$("#stackchart").bind("plotclick", function (event, pos, item) {
			if (item) {
				if (item.series.label == "3 Months"){
					$("#clickdata").text("Transactions from: " + country1[item.dataIndex]);
					plot.highlight(item.series, item.datapoint);}
				if (item.series.label == "6 Months"){
					$("#clickdata").text("Transactions from: " + country2[item.dataIndex]);
					plot.highlight(item.series, item.datapoint);}
				if (item.series.label == "12 Months"){
					$("#clickdata").text("Transactions from: " + country3[item.dataIndex]);
					plot.highlight(item.series, item.datapoint);}
			}
		});
	...
	/* ---------- Pie chart ---------- */
	var data = [
	{ label: lab4,  data: three},
	{ label: lab5,  data: six},
	{ label: lab6, data: twelve}
	];
